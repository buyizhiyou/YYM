...
def fgsm_attack(model, images, labels, epsilon=8/255, device=None, normalized=True):
...
def bim_attack(model, images, labels, epsilon=8/255,device=None, normalized=True):
...
def pgd_attack(model, images, labels, epsilon=8/255, device=None,normalized=True):
    attack = torchattacks.PGD(
        model, eps=epsilon, alpha=1/255, steps=10, random_start=True)
    if device:
        attack.set_device(device)
    if normalized:
        attack.set_normalization_used(
...
def test_adv(model:nn.Module, device:str, test_loader:DataLoader, epsilon:float = 0.01, method: str = "fgsm"):
    ...
    for images, target in test_loader:
        ...
        if final_pred.item() == target.item():
            ...
            if epsilon == 0 and len(adv_examples) < 5:# Special case for saving 0 epsilon examples
                adv_examples.append((images.cpu().detach().numpy().squeeze(0), perturbed_images.cpu().detach().numpy().squeeze(0), init_pred.item(
                ), final_pred.item(), clean_prob.item(), adv_prob.item()))
        else:
            if len(adv_examples) < 5: # Save 5 adv examples for visualization later
                adv_examples.append((images.cpu().detach().numpy().squeeze(0), perturbed_images.cpu().detach().numpy().squeeze(0), init_pred.item(
                ), final_pred.item(), clean_prob.item(), adv_prob.item()))
