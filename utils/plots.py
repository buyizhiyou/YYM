...
def plot_dist(axes: Axes, data_list: List[list], datanames: List[str], label: str = None):
    """绘制分布
...
def visualize_adv(x: np.ndarray, x_adv: np.ndarray, clean_pred: int, adv_pred: int,
                  clean_prob: float, adv_prob: float, mean: list, std: list):
    """可视化对抗样本
    ...
    ax[0].text(0.5, -0.2, "Pred: {}\n Prob: {:.2f}".format(clean_pred,
               clean_prob), size=10, ha="center", transform=ax[0].transAxes)
    ax[1].text(0.5, -0.2, "Pred: {}\n Prob: {:.2f}".format(adv_pred,
