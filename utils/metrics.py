...
def mutual_info(probs: np.ndarray):
    ...
    return entropy(np.mean(probs, axis=1), axis=1) - np.mean(
        entropy(probs, axis=2), axis=1)
...
def expected_kl(probs: np.ndarray):
    ...
    assert len(probs.shape) == 3, f"probs shape {probs.shape} is wrong"
    kl = entropy(
        np.mean(probs, axis=1)[..., None, :], probs + 1e-30,
        axis=2)  #+1e-30 to avoid divided by 0 and get inf  in KL divergence
...
def predictive_variance(probs: np.ndarray):
    ...
    assert isinstance(probs, np.ndarray), "probs should be np.array"
    assert len(probs.shape) == 3, f"probs shape {probs.shape} is wrong"
    variance = np.linalg.norm(probs - np.mean(probs, axis=1)[..., None, :],
                              ord=2,
                              axis=2)
...
def ece(targets: np.ndarray, y_pred: np.ndarray):
    ...
    ece_score = MulticlassCalibrationError(num_classes=10,
                                           n_bins=15,
                                           norm='l2')(torch.Tensor(y_pred),
