...
class SpectralNorm:
    ...
    def __init__(self, name: str = 'weight', n_power_iterations: int = 1, dim: int = 0, eps: float = 1e-12, coeff: float = 3.0) -> None:
        self.name = name
        self.dim = dim
    ...
    @staticmethod
    def apply(module: Module, name: str, n_power_iterations: int, dim: int, eps: float, coeff: float) -> 'SpectralNorm':
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNorm) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                   "the same parameter {}".format(name))

        fn = SpectralNorm(name, n_power_iterations, dim, eps, coeff)
        weight = module._parameters[name]
        if weight is None:
            raise ValueError(f'`SpectralNorm` cannot be applied as parameter `{name}` is None')
        if isinstance(weight, torch.nn.parameter.UninitializedParameter):
            raise ValueError('The module passed to `SpectralNorm` can\'t have uninitialized parameters. '
        ...
        module.register_forward_pre_hook(fn)  #这里，在每次进行module前向之前，先执行fn,谱归一化
...
def spectral_norm(module: T_module,
    ...
    if dim is None:
        if isinstance(module, (torch.nn.ConvTranspose1d, torch.nn.ConvTranspose2d, torch.nn.ConvTranspose3d)):
            ...
            dim = 0
    SpectralNorm.apply(module, name, n_power_iterations, dim, eps, coeff)
    return module
