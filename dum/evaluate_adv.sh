#! /bin/bash
echo "###########  Evaluate for advasarial samples with sn and mod;
###########  Usage:
###########   ./evaluate_adv.sh --gpu  0 --run 17 --batchsize 512 --evaltype gmm --model vgg16  --perturbation fgsm --contrastive 0 --adv 0"

# 解析命令行参数
options=$(getopt -o g:r:b:t:m:c:a --long gpu:,run:,batchsize:,evaltype:,model:,perturbation:,contrastive:,adv:, -- "$@")
eval set -- "$options"
while true; do
       case $1 in
       -g | --gpu)
...
perturbations=("fgsm" "bim" "pgd")
if [[ "$perturbation" = "all" ]]; then
       echo "evaluate for all attack"
       for attack in ${perturbations[@]}; do
