    ...
    for i in range(args.runs):
        ...
        if args.model_type == "ensemble":
            (
                ...
                predictions,
                confidences,
            ) = test_classification_net_ensemble(net_ensemble, test_loader,
                                                 device)
            ...
            (_, _, _), (_, _, _), m1_auroc, m1_auprc = get_roc_auc_ensemble(
                net_ensemble, test_loader, ood_test_loader,
                "mutual_information", device)
            ...
            (
                ...
                t_predictions,
                t_confidences,
            ) = test_classification_net_ensemble(t_ensemble, test_loader,
                                                 device)
            ...
            (_, _, _), (_, _,
                        _), t_m1_auroc, t_m1_auprc = get_roc_auc_ensemble(
                            t_ensemble, test_loader, ood_test_loader,
                            "mutual_information", device)
            (_, _,
             _), (_, _, _), t_m2_auroc, t_m2_auprc = get_roc_auc_ensemble(
